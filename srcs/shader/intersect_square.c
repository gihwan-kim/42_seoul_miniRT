...
/*   Updated: 2020/05/29 10:40:42 by gihwan-kim       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_sq	*intersection_square(t_rt *rt_info, t_ray *camera_ray, double *t)
{
	...
	// 	return (SUCCESS);
	if (get_object(rt_info->lst_pos.cur_sq))
		cur_square = get_object(rt_info->lst_pos.cur_sq)->content;
	else
		return (NULL);
	denominator = dot_product(&(camera_ray->direction_), &(cur_square->orient_vec_));
	if (denominator > 10e-7)
	{
		...
		*t = dot_product(&p0_l0, &(cur_square->orient_vec_)) / denominator;
		if (*t >= 0)
		{
			multi_result = multiply(&(camera_ray->direction_), *t);
			p = add(&(camera_ray->origin_), &multi_result);
