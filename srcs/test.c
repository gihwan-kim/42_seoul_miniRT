...
// #include "mlx.h"
# include <math.h>
# include "../lib/minilibx_mms_20200219_beta/minilibx_mms_20200219/mlx.h"
...
int	main(void)
{
	...
	*/
	mlx.img.data = (int *)mlx_get_data_addr(mlx.img.img_ptr, &mlx.img.bpp, &mlx.img.size_l,
		&mlx.img.endian);
	printf("bpp : %d\t size_l : %d\t endoam : %d\n", mlx.img.bpp, mlx.img.size_l, mlx.img.endian);
	/*
	...
	while (++count_h < WIN_HEIGHT)
	{
		...
		while (++count_w < WIN_WIDTH)
		{
			if (count_w % 2)
				...
				*/
				mlx.img.data[count_h * WIN_WIDTH + count_w] = 1;
			else
				mlx.img.data[count_h * WIN_WIDTH + count_w] = 0;
		}
	}
	printf("std rgb color : %u\n", mlx_get_color_value(mlx.mlx_ptr, 255));
